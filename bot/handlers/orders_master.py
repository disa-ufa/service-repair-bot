...
async def master_cash(message: Message):
    ...
    pending = [o for o in rows if not getattr(o, "payout_screenshot_file_id", None)]
    if len(pending) >= 2:
        total_pending = sum(int(o.company_amount or 0) for o in pending)
        kb_rows.insert(
            0,
            [
                InlineKeyboardButton(
                    text=f"💸 Сдать всё одним скрином ({total_pending} ₽)",
                    callback_data="cash_payout_all",
                )
            ],
        )
...
async def _finalize_close(message: Message, state: FSMContext, bot: Bot, cfg: Config, tg_id: Optional[int] = None):
    ...
    if getattr(cfg, "business", None):
        req_text = (getattr(cfg.business, "payout_requisites_text", "") or "").strip()
        if req_text:
...
# ------------------------
@router.callback_query(F.data == "cash_payout_all")
async def cash_payout_all_start(cb: CallbackQuery, state: FSMContext, cfg: Config):
    if not cb.message:
        return
    ...
    pending = [o for o in rows if not getattr(o, "payout_screenshot_file_id", None)]
    if len(pending) < 2:
        await cb.answer("Нет нескольких заявок без скрина", show_alert=True)
        return

    total = sum(int(o.company_amount or 0) for o in pending)
    ids = [int(o.id) for o in pending]

    await state.clear()
    await state.update_data(order_ids=ids, payout_total=total)
    ...
    lines = [
        "Отправьте <b>фото</b> со скрином перевода (одним сообщением).",
        f"\n<b>Сумма:</b> {total} ₽",
        "<b>Заявки:</b>",
    ]
    for o in pending:
        lines.append(f"• #{o.id} — к сдаче: <b>{int(o.company_amount or 0)} ₽</b>")

    await cb.message.answer("\n".join(lines) + req, parse_mode=ParseMode.HTML)
...
async def order_payout_save(message: Message, state: FSMContext, bot: Bot, cfg: Config):
    ...
    # -------------------------
    # BULK: один скрин на несколько заявок из "Кассы"
    # -------------------------
    ...
    if order_ids:
        ...
        for oid in ids:
            o = await repo.attach_order_payout_proof(oid, u, file_id=file_id)
            if o:
                ok.append(o)
            else:
                failed.append(oid)
        ...
        if not ok:
            await message.answer("Не удалось сохранить скрин (возможно заявки не ваши или уже не в статусе 'Требуется сдача').")
            return

        total = sum(int(getattr(o, "company_amount", 0) or 0) for o in ok)
        ok_ids = ", ".join(f"#{o.id}" for o in ok)

        txt = (
            f"✅ Скрин принят по заявкам: <b>{ok_ids}</b>\n"
            f"Сумма сдачи: <b>{total} ₽</b>. Ожидаем подтверждение администратора."
        )
        if failed:
            txt += "\n\n⚠️ Не удалось применить скрин к заявкам: " + ", ".join(f"#{i}" for i in failed)

        await message.answer(txt, parse_mode=ParseMode.HTML)

        # Уведомление админам (с перечнем заявок)
        lines = [
            f"💸 Мастер <b>{html_escape(u.fio)}</b> отправил <b>ОДИН</b> скрин общей сдачи.",
            f"Сумма: <b>{total} ₽</b>",
            "",
            "<b>Заявки:</b>",
        ]
        for o in ok:
            lines.append(f"• <b>#{o.id}</b> — к сдаче: <b>{int(getattr(o, 'company_amount', 0) or 0)} ₽</b>")

        kb_rows: list[list[InlineKeyboardButton]] = []
        for o in ok:
            kb_rows.append(
                [
                    InlineKeyboardButton(text=f"Открыть #{o.id}", callback_data=f"order_view:{o.id}"),
                    InlineKeyboardButton(text=f"✅ Подтв. #{o.id}", callback_data=f"order_admin_paid:{o.id}"),
                ]
            )
        ...
        await _notify_admins(
            ...
            cfg,
            "\n".join(lines),
            reply_markup=_kb_inline(kb_rows) if kb_rows else None,
        )
        return

    # -------------------------
    # SINGLE: скрин по одной заявке
    # -------------------------
    order_id = int(data.get("order_id") or 0)
